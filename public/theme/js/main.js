...
(function () {
  ...
  Riode.defaults = {
    ...
    templateCartAddedAlert:
      ...
      `<a href="/mon-panier" class="btn btn-success btn-md">Voir le panier</a>` +
  ...
  Riode.initProductSingle = (function () {
    ...
    var initCartAction = function (self) {
      ...
      self.$wrapper.on("click", ".btn-cart", function (e) {
        ...
        // console.log($product.find(".product-name > a").attr("href"));
        ...
        if (
          ...
          Riode.Minipopup.open({
            ...
            actionTemplate:
              `<div class="action-group d-flex mt-3"><a href="/mon-panier" class="btn btn-sm btn-outline btn-primary btn-rounded mr-2">Voir le panier</a><a href="/checkout" class="btn btn-sm btn-primary btn-rounded"> Départ</a></div>`,
          });
        }
      });
    };
  ...
  Riode.initProductSinglePage = (function () {
    ...
    function compareAdded(e) {
      ...
      console.log($product.find("h1.product-name").text());
      if (
          ...
          function (e) {
            ...
            if ($compareCol.length > 0) {
              ...
              $productName = $(".riode-compare-table > .compare-row")
                ...
                console.log($productName.find("a").text());
                ...
                Riode.Minipopup.open({
                  ...
                  count:
                    $product.find(".quantity").length > 0
                      ...
                      : 1,
                  actionTemplate:
                    `<div class="action-group d-flex"><a href="/mon-panier" class="btn btn-sm btn-outline btn-primary btn-rounded">Voir le panier</a><a href="/checkout" class="btn btn-sm btn-primary btn-rounded">Vérifier</a></div>`,
                });
            } else {
              // if not product single, then open minipopup
  ...
  <div class="product-detail">
    ...
    <div class="price-box">
      ...
      <span class="product-price">€${produit.prix_promo ? produit.prix_promo : produit.prix_regulier}</span>
              ...
              // gestion de la mise sous panier
              let produit_simple_id = $product.find(".product-id").text()
              let produit_simple_photo = $product.find(".product-media img").attr("src") ? $product.find(".product-media img").attr("src") : $product.find(".produit-link").text() 
              let produit_simple_prix_promo = $product.find(".product-price > .promo-price").text()
              let produit_simple_prix_regulier = $product.find(".product-price > .prix-regulier").text()
              let produit_simple_quantity = $product.find(".quantity").length > 0 ? $product.find(".quantity").val() : 1

              console.log('produit-simple id : ' , produit_simple_id );
              console.log('produit_simple_photo : ', produit_simple_photo)

              // save in cart 
              var current_panier_content = []
              current_panier_content = localStorage.getItem('panier') ? JSON.parse(localStorage.getItem('panier') ): []

              var update_panier = [];

              current_panier_content.map((produit,i) => {
                if ( produit.produit_id != produit_simple_id ) {
                  update_panier.push(produit)
                } 
              });

              update_panier.push({
                produit_id : produit_simple_id,
                quantite : produit_simple_quantity,
                type:'1', // simple,
                image : produit_simple_photo,
                prix_promo: produit_simple_prix_promo,
                prix_regulier: produit_simple_prix_regulier,
                nom: $product.find(".product-name").text()
              })
              localStorage.setItem('panier', JSON.stringify(update_panier))
